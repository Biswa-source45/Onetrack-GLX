...
import {
  ...
  getBidEndDate,
  formatDateTime,
  getTargetMonthDisplay,
  getSubmissionStatusVal,
  getFinEvalStatusVal,
  getPoRecvStatusVal,
  getBidResultVal,
  escapeCSV,
...
const STATUS_OPTIONS = [
  ...
  {
    value: "STAGE_TECHNICAL_EVALUATION",
    label: "Under Tech Eval",
    stage: "TECHNICAL_EVALUATION",
  },
  {
    value: "STAGE_FINANCIAL_EVALUATION",
    label: "Under Fin Eval",
...
const PORTAL_SOURCES = ["GeM", "Private", "RTC", "CPPP", "eProcure"];
...
export function TendersPage({ initialScope = "all" }) {
  ...
  const isOwnedView =
    location.pathname.includes("/dashboard/tenders/owned") ||
    initialScope === "owned";
  ...
  const loadMoreAuditLogs = useCallback(async () => {
    ...
    try {
      const res = await getGlobalAuditHistory({
        limit: 30,
        cursor: auditCursor,
      });
      if (res.ok && Array.isArray(res.data)) {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting) loadMoreAuditLogs();
      },
  ...
  const toggleSheetEditable = () => {
    ...
    if (nextState) {
      toast.success("Edit Mode Enabled: Autosave activated", {
        id: "autosave-status",
      });
    } else {
      toast.success("Edit Mode Disabled: Autosave deactivated", {
        id: "autosave-status",
      });
    }
  };
  ...
  useEffect(() => {
    ...
    const sourceParam = searchParams.get("source") || searchParams.get("portal_source");
    ...
    if (sourceParam !== null && sourceParam !== portalSourceFilter) {
      setPortalSourceFilter(sourceParam);
      hasParamUpdate = true;
    }
  ...
  const handlePermanentDeleteBid = async (bidId, bidTitle) => {
    if (
      !window.confirm(
        `Are you sure you want to PERMANENTLY delete tender "${bidTitle || bidId}"? This action CANNOT be undone.`,
      )
    ) {
      return;
    }
    try {
      ...
      if (res.ok) {
        ...
        toast.error(
          res.error?.message ?? "Failed to permanently delete tender",
        );
      }
    } catch (err) {
  ...
  const handleFieldSave = async (bidId, field, value) => {
    ...
    if (field === "workflow_stage") {
      try {
        const res = await transitionBidStage(
          bidId,
          value,
          "Updated via spreadsheet inline edit",
        );
        if (res.ok) {
          const updated = bids.map((b) => {
            if (b.id === bidId) {
          ...
          toast.error(
            res.error?.message ??
              `Invalid transition to ${STAGE_LABELS[value] ?? value}`,
          );
          loadBids();
        }
      } catch (err) {
    ...
    const payload = {
      ...
      estimated_value:
        currentBid.estimated_value !== null
          ? Number(currentBid.estimated_value)
          : null,
      emd_amount:
        currentBid.emd_amount !== null ? Number(currentBid.emd_amount) : null,
      ...
      opening_date: currentBid.opening_date
        ? new Date(currentBid.opening_date).toISOString()
        : null,
      closing_date: currentBid.closing_date
        ? new Date(currentBid.closing_date).toISOString()
        : null,
      ...
      target_month_date: currentBid.target_month_date
        ? new Date(currentBid.target_month_date).toISOString()
        : null,
  ...
  const handlePoReceivedToggle = async (bidId, checked) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(
          checked ? "PO marked as Received" : "PO status set to Pending",
        );
      } else {
    ...
    <div className="space-y-6 w-full min-w-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-bold text-rose-900 dark:text-rose-200">
                Tender Bin (15-Day Retention Period)
              </p>
              <p className="text-xs text-rose-700 dark:text-rose-400 mt-0.5">
                Items in this bin will be permanently purged automatically after
                15 days of soft-deletion. Super Admins can restore tenders back
                to the active workspace at any time.
              </p>
            </div>
          </div>
          <Button
            ...
            className="text-xs shrink-0 bg-card border-rose-300 hover:bg-rose-100/50"
          >
            ...
            <motion.div
              ...
              onClick={() => {
                if (stat.filterKey === "") {
                  ...
                  setSearchParams(
                    inBin
                      ? { bin: "true", status: stat.filterKey }
                      : { status: stat.filterKey },
                  );
                }
              }}
        ...
        <div className="flex flex-wrap gap-2.5 items-center justify-between">
          <div className="flex flex-wrap gap-2.5 items-center flex-1 min-w-[280px]">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="outline"
                  size="sm"
                  className="h-8.5 text-xs font-normal bg-background border-input text-foreground hover:bg-muted/50 gap-1.5"
                >
                  <span>
                    {STAGE_OPTIONS.find((o) => o.value === stageFilter)
                      ?.label || "All Stages"}
                  </span>
            ...
            <DropdownMenu>
                ...
                >
                  <span>
                    {(
                      (statusFilter
                        ? STATUS_OPTIONS.find(
                            (o) => !o.stage && o.value === statusFilter,
                          )
                        : null) ||
                      STATUS_OPTIONS.find(
                        (o) => o.stage && o.stage === stageFilter,
                      )
                    )?.label || "All Statuses"}
                  </span>
                  <ChevronDown className="size-3 text-muted-foreground ml-auto" />
              ...
              <DropdownMenuContent>
                ...
                <DropdownMenuSeparator />
                {STATUS_OPTIONS.filter((o) => o.value !== "ARCHIVED").map(
                  (o) => (
                    <DropdownMenuItem
                      key={o.value}
                      onSelect={() => {
            ...
            {/* Tender Source / Portal Source filter */}
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="outline"
                ...
                >
                  ...
                  <span>
                    {PORTAL_SOURCE_OPTIONS.find(
                      (o) => o.value === portalSourceFilter,
                    )?.label || "All Sources"}
                  </span>
                  <ChevronDown className="size-3 text-muted-foreground ml-auto" />
                </Button>
              ...
              <DropdownMenuContent className="max-h-60 overflow-y-auto">
                <DropdownMenuLabel>Filter by Tender Source</DropdownMenuLabel>
                <DropdownMenuSeparator />
                {PORTAL_SOURCE_OPTIONS.map((o) => (
                  <DropdownMenuItem
                    key={o.value}
                    onSelect={() => {
                      setPortalSourceFilter(o.value);
                      setPage(1);
                    }}
                  >
                    {o.label}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                ...
                >
                  ...
                  <span>
                    {END_DATE_OPTIONS.find((o) => o.value === endDateFilter)
                      ?.label || "All End Dates"}
                  </span>
                  <ChevronDown className="size-3 text-muted-foreground ml-auto" />
                </Button>
              ...
              <DropdownMenuContent>
                ...
                {END_DATE_OPTIONS.map((o) => (
                  <DropdownMenuItem
                    key={o.value}
                    onSelect={() => setEndDateFilter(o.value)}
                  >
                    {o.label}
                  </DropdownMenuItem>
                ))}
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    ...
                    <span className="max-w-[140px] truncate">
                      {ownerFilterId
                        ? (ownerStats.find((s) => s.user_id === ownerFilterId)
                            ?.full_name ?? "Owner")
                        : "All Owners"}
                    </span>
                    <ChevronDown className="size-3 text-muted-foreground ml-auto" />
                ...
                <DropdownMenuContent className="max-h-72 overflow-y-auto w-[260px]">
                  ...
                  {ownerStats.map((s) => (
                    <DropdownMenuItem
                      key={s.user_id}
                      onSelect={() => setOwnerFilterId(s.user_id)}
                    >
          ...
          <div className="flex items-center gap-2">
            {viewMode === "sheets" && hasPermission("bid.edit") && (
              ...
              >
                <Zap
                  className={`size-3.5 ${sheetEditable ? "animate-pulse text-white" : "text-muted-foreground"}`}
                />
            ...
            {viewMode === "sheets" && (
              ...
              >
                <History
                  className={`size-3.5 ${showAuditPanel ? "text-white" : "text-muted-foreground"}`}
                />
                {sheetAuditLog.length > 0
                  ? `History (${sheetAuditLog.length})`
                  : "History"}
              </button>
            )}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 gap-3 text-muted-foreground bg-card border border-border rounded-xl">
          ...
          <Button variant="outline" size="sm" onClick={loadBids}>
            Retry Query
          </Button>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <AnimatePresence initial={false}>
                {effectiveBids.map((bid, i) => {
                  ...
                  const progressPct = TERMINAL_STAGES.includes(
                    bid.workflow_stage,
                  )
                    ? 100
                    : currentStageIdx >= 0
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        className={`absolute top-0 left-0 right-0 h-1 ${statusStyle(bid.bid_status).accent}`}
                      />
                      {/* Stamp watermark for terminal status tenders */}
                      {["CANCELLED", "CLOSED", "WON", "LOST"].includes(
                        bid.bid_status,
                      ) && (
                        <div className="pointer-events-none absolute inset-0 flex items-center justify-center z-10 select-none">
                          <span
                            className={`text-4xl font-black tracking-widest rotate-[-25deg] opacity-[0.13]
                        ...
                        <div>
                          <div className="flex items-start gap-1.5 justify-between">
                            ...
                            {bid.bid_status === "ACTIVE" ? (
                              <span
                                className="relative flex size-2 mt-1 shrink-0"
                                title="Active"
                              >
                              ...
                              <span
                                className="size-2 rounded-full bg-sky-500 mt-1 shrink-0"
                                title="Won"
                              />
                            ) : bid.bid_status === "LOST" ? (
                              <span
                                className="size-2 rounded-full bg-orange-500 mt-1 shrink-0"
                                title="Lost"
                              />
                            ) : (
                              <span
                                className="size-2 rounded-full bg-red-600 mt-1 shrink-0"
                        ...
                        <div className="space-y-1.5 bg-muted/30 p-2.5 rounded-lg border border-border/40 text-xs">
                          <div className="flex justify-between items-center">
                            <span className="text-muted-foreground flex items-center gap-1">
                              <Building2 className="size-3 text-muted-foreground/75" />{" "}
                              Account Name
                            </span>
                          ...
                          <div className="flex justify-between items-center">
                            <span className="text-muted-foreground flex items-center gap-1">
                              <DollarSign className="size-3 text-muted-foreground/75" />{" "}
                              Est. Value
                            </span>
                          ...
                          {bid.account_manager && (
                            <div className="flex justify-between items-center">
                              <span className="text-muted-foreground flex items-center gap-1">
                                <UserCheck className="size-3 text-muted-foreground/75" />{" "}
                                Account Manager
                              </span>
                              <span className="font-semibold text-foreground truncate max-w-[150px]">
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <span className="font-bold text-primary text-[11px]">
                              {STAGE_LABELS[bid.workflow_stage] ??
                                bid.workflow_stage}
                            </span>
                          </div>
                      ...
                      <div className="mt-4 pt-3 border-t border-border flex items-center justify-between text-xs text-muted-foreground">
                        ...
                        {bid.bid_owner && (
                          <div
                            ...
                            title={`Owner: ${bid.bid_owner.full_name}`}
                          >
                            ...
                            <span className="max-w-[70px] truncate">
                              {bid.bid_owner.full_name.split(" ")[0]}
                            </span>
                          </div>
                        )}
                      </div>
                  ...
                  <table className="min-w-full w-max text-left border-collapse text-xs">
                    <thead>
                      <tr className="bg-muted/40 border-b border-border text-[11px] font-bold text-muted-foreground uppercase tracking-wider whitespace-nowrap">
                        <th className="p-3 border-r border-border min-w-[240px] sticky left-0 bg-[color-mix(in_srgb,var(--muted)_40%,var(--card))] z-20 shadow-[2px_0_8px_-2px_rgba(0,0,0,0.15)]">
                          Tender Title
                        </th>
                        <th className="p-3 border-r border-border min-w-[120px]">
                          Status
                        </th>
                        <th className="p-3 border-r border-border min-w-[180px]">
                          Workflow Stage
                        </th>
                        <th className="p-3 border-r border-border min-w-[120px]">
                          Category
                        </th>
                        <th className="p-3 border-r border-border min-w-[140px]">
                          Bid ID
                        </th>
                        <th className="p-3 border-r border-border min-w-[135px]">
                          Platform
                        </th>
                        <th className="p-3 border-r border-border min-w-[180px]">
                          Department
                        </th>
                        <th className="p-3 border-r border-border min-w-[240px]">
                          High Level Scope
                        </th>
                        <th className="p-3 border-r border-border min-w-[90px]">
                          Quantity
                        </th>
                        <th className="p-3 border-r border-border min-w-[140px]">
                          Scope Type
                        ...
                        <th className="p-3 border-r border-border min-w-[150px]">
                          Target Month
                        </th>
                        <th className="p-3 border-r border-border min-w-[150px]">
                          Start Date
                        ...
                        <th className="p-3 border-r border-border min-w-[160px]">
                          Fin Eval Status
                        </th>
                        <th className="p-3 border-r border-border min-w-[140px]">
                          PO Received
                        ...
                        <th className="p-3 border-r border-border min-w-[200px]">
                          Remarks
                        </th>
                        {inBin && (
                          <th className="p-3 min-w-[160px] text-center bg-rose-50/50 text-rose-900">
                            Bin Actions
                          </th>
                        )}
                      </tr>
                    ...
                    <tbody className="divide-y divide-border">
                      {effectiveBids.map((bid) => {
                        ...
                        const isReadOnly =
                          !sheetEditable || bid.bid_status === "ARCHIVED";
                        ...
                        const bidIdField = bid.gem_bid_no
                          ? "gem_bid_no"
                          : bid.bid_no
                            ? "bid_no"
                            : "gem_bid_no";
                        return (
                          ...
                          >
                            ...
                            <td className="p-3 border-r border-border text-center">
                              <span
                                className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ${statusStyle(bid.bid_status).pill}`}
                              >
                                {bid.bid_status}
                              </span>
                            </td>
                                ...
                                <DropdownMenu>
                                  ...
                                  >
                                    {STAGE_OPTIONS.filter(
                                      ...
                                      const isCurrent =
                                        o.value === bid.workflow_stage;
                                      const allowedNext =
                                        STAGE_TRANSITIONS[bid.workflow_stage] ??
                                        [];
                                      const isAllowed =
                                        isCurrent ||
                                ...
                                <DropdownMenu>
                                  <DropdownMenuTrigger asChild>
                                    <button className="flex items-center gap-1.5 bg-background hover:bg-muted/50 text-foreground border border-border rounded-md px-2 py-1 text-xs text-left min-w-[90px] justify-between focus:ring-1 focus:ring-primary outline-none transition-all shadow-sm">
                                      <span>
                                        {bid.portal_source || "Select Portal"}
                                      </span>
                                      <ChevronDown className="size-3 text-muted-foreground ml-1" />
                                    </button>
                                  </DropdownMenuTrigger>
                            ...
                            <td className="p-3 border-r border-border">
                              {!isReadOnly ? (
                                ...
                                <span
                                  className="text-emerald-600 font-bold flex items-center gap-0.5"
                                  title={formatEmdExemption(bid)}
                                >
                                  <Check className="size-3.5" />{" "}
                                  {formatEmdExemption(bid)}
                                </span>
                              ) : (
                                <span className="text-muted-foreground">
                                  No
                            ...
                            <td className="p-3 border-r border-border text-foreground font-medium">
                              {!isReadOnly ? (
                                <input
                                  type="date"
                                  value={safeDateInputFormat(
                                    bid.target_month_date,
                                  )}
                                  onChange={(e) =>
                                    handleImmediateChangeAndSave(
                                      bid.id,
                            ...
                            <td className="p-3 border-r border-border font-mono font-semibold text-foreground">
                              {bid.quoted_price !== null &&
                              bid.quoted_price !== undefined
                                ? formatCurrency(bid.quoted_price)
                                : "—"}
                            </td>
                            ...
                            <td className="p-3 border-r border-border text-muted-foreground">
                              {formatDate(bid.po_received_date)}
                            </td>
                            <td className="p-3 border-r border-border text-muted-foreground">
                              {formatDate(bid.emd_ready_date)}
                            </td>
                            <td className="p-3 border-r border-border text-muted-foreground">
                              {formatDate(bid.bg_discharged_date)}
                            </td>
                            <td className="p-3 border-r border-border text-muted-foreground">
                              {formatDate(bid.delivery_complete_date)}
                            </td>
                            ...
                            <td className="p-3 border-r border-border">
                              {!isReadOnly ? (
                                <DropdownMenu>
                                  ...
                                  <DropdownMenuContent
                                    align="start"
                                    className="max-h-[200px] overflow-y-auto min-w-[140px]"
                                  >
                                    <DropdownMenuItem
                                      onSelect={() =>
                                        handleImmediateChangeAndSave(
                            ...
                            {/* 24. Remarks */}
                            <td
                              className={`p-3 truncate max-w-xs text-muted-foreground ${inBin ? "border-r border-border" : ""}`}
                              title={bid.remarks}
                            >
                              {!isReadOnly ? (
                                <input
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between px-3.5 py-2.5 border-b border-indigo-200 dark:border-indigo-900/60 bg-indigo-100/80 dark:bg-indigo-900/40 backdrop-blur-sm">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-indigo-900 dark:text-indigo-200 tracking-wide">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        >
                          <RefreshCw
                            className={`size-3.5 ${loadingAudit ? "animate-spin" : ""}`}
                          />
                        </button>
                        <button
                    ...
                    <div className="overflow-y-auto flex-1 divide-y divide-indigo-100 dark:divide-indigo-900/40">
                      {loadingAudit ? (
                        ...
                        <div className="p-6 text-center text-xs text-muted-foreground">
                          ...
                          <br />
                          Edit tender fields to create audit entries.
                        </div>
                      ) : (
                        sheetAuditLog.map((entry, i) => {
                          ...
                          const dateStr = entry.created_at
                            ...
                            : new Date();
                          const dayLabel = entry.created_at
                          ...
                          const isNewDay =
                            ...
                            (!prevEntry?.created_at ||
                              dateGroupLabel(new Date(prevEntry.created_at)) !==
                                dayLabel);
                            ...
                            <div key={entry.id}>
                              ...
                              <div className="px-3.5 py-2.5 text-[11px] space-y-1.5 hover:bg-indigo-100/50 dark:hover:bg-indigo-900/20 transition-colors">
                                <div className="flex items-center justify-between">
                                  <span
                                    className="font-semibold text-indigo-900 dark:text-indigo-300 truncate max-w-[170px]"
                                    title={entry.bid_title}
                                  >
                                    {entry.bid_title}
                                  </span>
                                  <span className="text-muted-foreground text-[10px] shrink-0 ml-1">
                                    {dateStr.toLocaleTimeString("en-IN", {
                                      hour: "2-digit",
                                      minute: "2-digit",
                                      hour12: true,
                                    })}
      ...
      {!loading && effectiveBids.length > 0 && (
        <div className="flex items-center justify-between px-4 py-3 border border-border bg-card rounded-xl shadow-sm">
          <p className="text-xs text-muted-foreground">
            Showing {(page - 1) * (meta.limit ?? 20) + 1}–
            {Math.min(page * (meta.limit ?? 20), meta.total ?? 0)} of{" "}
            {meta.total ?? 0} tenders
          </p>
          <div className="flex items-center gap-1">
            <Button
              variant="outline"
